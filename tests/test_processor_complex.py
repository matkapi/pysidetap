...
def test_DTProcessor_reservoar1():
    ...
    r = p.process({'reservoar_temp': 47.0, 'cooldown': 60 * 60, 'heat_status': 'off'})
    assert r == {
        'heat': 'on',
...
def test_DTProcessor_reservoar2():
    ...
    p = DTProcessor(DTableReservoar)
    r = p.process({'reservoar_temp': 49.0, 'cooldown': 60, 'heat_status': 'on'})
    assert r == {
        'heat': 'on',
...
def test_DTProcessor_reservoar3():
    ...
    p = DTProcessor(DTableReservoar)
    r = p.process({'reservoar_temp': 56.0, 'cooldown': 29 * 60, 'heat_status': 'on'})
    assert r == {
        'heat': 'on',
...
def test_DTProcessor_reservoar4():
    """Test when reservoar temp is over 56."""
    p = DTProcessor(DTableReservoar)
    r = p.process({'reservoar_temp': 56.0, 'cooldown': 31 * 60, 'heat_status': 'on'})
    assert r == {
        'heat': 'on',
        'message': 'reservoar heat_status was on and temp was over '
        'max_temp and warmup is not passed',
...
def test_DTProcessor_reservoar5():
    ...
    p = DTProcessor(DTableReservoar)
    r = p.process({'reservoar_temp': 54.0, 'cooldown': 2 * 60, 'heat_status': 'off'})
    assert r == {'heat': 'off', 'message': 'something else then heat off'}
